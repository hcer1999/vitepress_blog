...
!(function (D, u) {
  ...
  function m(D) {
    ;(U = document),
      (i = window),
      (s = s || D || i.gsap || console.warn('Please gsap.registerPlugin(SplitText)')) &&
        ((d = s.utils.toArray), (r = s.core.context || function () {}), (n = 1))
  }
  function p() {
  ...
  function D(D, e, F, i, n, s, E) {
        ...
        if (
          ...
          if (p._isSplit)
            for (
              ...
              (p._next = o.nextSibling), p.parentNode.appendChild(o), (o = o.nextSibling)
          else
          ...
          p.parentNode._isSplit && !p._isSplit && '' !== p.innerHTML
            ? H.push(p)
            : $ && !p._isSplit && ('span' === V && (p.style.display = 'inline'), O.push(p))
      else
        R || q
        ...
        for (
          h = R[r],
            (g = U.createElement(V)).style.cssText =
              'display:block;text-align:' + W + ';position:' + (q ? 'absolute;' : 'relative;'),
          ...
          'BR' !== h[o].nodeName &&
            ...
            q &&
              (0 === o && ((g.style.top = p._y + 'px'), (g.style.left = _ + v + 'px')),
              (p.style.top = '0px'),
              v && (p.style.left = p._x - v + 'px')))
        0 === l ? (g.innerHTML = '&nbsp;') : P || $ || (B(g), y(g, String.fromCharCode(160), ' ')),
    ...
    q &&
      (E > D.clientHeight &&
        ((D.style.height = E - T + 'px'), D.clientHeight < E && (D.style.height = E + w + 'px')),
      s > D.clientWidth &&
        ((D.style.width = s - N + 'px'), D.clientWidth < s && (D.style.width = s + m + 'px'))),
      ...
      if (((r = i.charAt(n)), A && (l = A(i.substr(n), e.specialChars))))
        ...
        '{' === r && '{{LT}}' === i.substr(n, 6)
          ...
          ? ((s = ((i.substr(n, 12).split(S) || [])[1] || '').length || 2),
            (C += d && ' ' !== r ? F() + i.substr(n, s) + '</' + p + '>' : i.substr(n, s)),
            (n += s - 1))
          : (C += d && ' ' !== r ? F() + r + '</' + p + '>' : r)
  ...
  var U,
    ...
    f = (function (D) {
      var u = 'undefined' != typeof window,
        e =
          0 === (u ? window.location.href : '').indexOf(p(102, 105, 108, 101, 58, 47, 47)) ||
